  ...
  <div class="game-container">
    ...
    <main class="mainboard">
      ...
      <div v-else class="game-content">
        ...
        <div class="gameDecks">
          <DeckComponent 
            title="Civis Deck (Board)"
            :count="gameState.decks?.civis?.count || 0"
            deck-type="civis"
            :deck-metadata="gameState.decks?.civis?.metadata || {}"
            @peek="peekDeck('civis')"
            @shuffle="shuffleDeck('civis')"
          />
          <DeckComponent 
            title="Others Deck"
            :count="gameState.decks?.others?.count || 0"
            deck-type="others"
            :deck-metadata="gameState.decks?.others?.metadata || {}"
            @draw="drawCard"
            @peek="peekDeck('others')"
            @shuffle="shuffleDeck('others')"
          />
        </div>

        <div class="players-board-container">
          <div v-for="playerState in gameState.players">      
            <div class="player-board" v-if="currentPlayer.id == playerState.id">
              <div class="playerField">
                <CardComponent 
                  v-for="cardId in playerState.field"
                  :key="cardId"
            ...
            <div class="player-board" v-else>
              ...
              <div class="opponentHand">
                <div class="opponentCard" v-for="card in playerState.hand"></div>
              </div>
            </div>
    ...
    <aside class="sidebar">
      <div class="game-stats">
        <h3 class="game-stats__title">Stats</h3>
        <div class="game-stats__content">
          <span class="game-stats__coins">Coins: {{ currentPlayer?.coins || 0 }}</span>
          <button 
            @click="addCoin"
            class="game-stats__add-button"
          >
            +1
          </button>
        </div>
      ...
      <div class="game-players">
        ...
        <div class="game-players__list">
          ...
          >
            ...
            <div class="game-players__stats">
              <div class="game-players__stat">Cards in hand: {{ player.hand?.length || 0 }}</div>
              <div class="game-players__stat">Cards on field: {{ player.field?.length || 0 }}</div>
              <div class="game-players__stat">Coins: {{ player.coins || 0 }}</div>
              <div v-if="player.stats" class="game-players__meta">
                Turns: {{ player.stats.turnsTaken || 0 }}
              </div>
            </div>
          </div>
        </div>
...
import { civisDeck as baseCivisDeck, othersDeck as baseDeck } from '../data/allCards.js'
...
const gameState = ref({
  ...
  decks: {
    civis: { cardIds: [], count: 0, metadata: {} },
    others: { cardIds: [], count: 0, metadata: {} }
  },
  board: { civisCards: [], othersCards: [], positions: {} },
...
const boardCivisCards = computed(() => {
  return gameState.value.board?.civisCards || []
})
...
async function drawCard() {
  ...
  const deck = gameState.value.decks?.others
...
function peekDeck(deckType, count = 5) {
  const deck = deckType === 'civis' ? gameState.value.decks?.civis : gameState.value.decks?.others
  if (!deck || deck.cardIds.length === 0) {
